...
SiteScribe assumes content is managed in git (and/or generated by SvelteKit), with AI coding agents acting as the “CMS interface.” SiteScribe supplies the missing piece: fast, accurate search with incremental indexing, branch scoping, and safe cost controls.
...
* All SiteScribe code is open source.
* External services (OpenAI embeddings, Pinecone, Milvus Cloud, Jina reranker) may be used via API; SiteScribe itself remains open source.
...
* **`sitescribe`**
  ...
  * CLI (`sitescribe` binary)
  * SvelteKit integration (`sitescribe/sveltekit`)
  * MCP server (`sitescribe mcp`)
...
### 5.2 Minimal config (`sitescribe.config.ts`)
...
SiteScribe generates and maintains a deterministic Markdown representation of each page under:

* `.sitescribe/pages/<scopeName>/...`
...
export default {
  ...
  vector: {
    ...
    local?: {
      path?: string;                // default ".sitescribe/local-vectors.sqlite"
    };
  },
...
3. Write Markdown mirror files to `.sitescribe/pages/<scope>/...`
...
`.sitescribe/pages/<scope>/docs/getting-started.md`
...
* `.sitescribe/embeddings-cache.sqlite` (preferred)
* Must support:
...
Maintain `.sitescribe/manifest.json` with:
...
* CLI must require explicit `sitescribe index --force` to migrate
...
* SQLite file: `.sitescribe/local-vectors.sqlite`
...
SiteScribe must map each indexed page to a SvelteKit route root file:
...
* Creates `sitescribe.config.ts` minimal
* Prints integration snippet for `src/hooks.server.ts`
* Creates `.sitescribe/` directory
...
* Deletes local `.sitescribe/` state
...
* Local: `.sitescribe/registry.json`
* Remote: lightweight registry record stored using the chosen backend strategy:

  * Pinecone: special namespace `_sitescribe_registry`
  * Milvus: separate collection `${projectId}_registry`

`sitescribe prune` relies on this for correctness.
...
1. Build site
2. `sitescribe index --changed-only`
...
* `sitescribe index --dry-run` on PRs to report embedding cost
* `sitescribe index` on merge to main

---

## 21. Cost Estimation

SiteScribe must estimate embedding cost before calling embeddings API:
...
* `sitescribe index --dry-run`
* `sitescribe status`
* `sitescribe index` preflight output
...
1. `sitescribe init` + one-line `hooks.server.ts` exposes `/api/search`.
2. `sitescribe index` builds `.sitescribe/pages/main/**.md`, chunks it, and upserts to Pinecone or Milvus Cloud.
...
6. `sitescribe prune` can list scopes and delete stale scopes safely (dry-run default).
7. `sitescribe mcp` returns results including the SvelteKit `+page.svelte` route root file.
...
* Markdown mirror directory: `.sitescribe/pages/<scope>/`
